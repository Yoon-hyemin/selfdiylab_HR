...
**포함**: Level1 문턱값 수정 API+모달, 공통 40점 배점 수정 API+모달(합계 40 서버 검증, **항목 추가/삭제 포함** — 사용자 확인: 5개 고정이 아니라 자유롭게 늘리고 줄일 수 있어야 함), 두 API가 공유하는 "새 버전 생성" 헬퍼.

**포함 안 함**: 직무 60점/임계값/하루상한 수정(1B-3), 초안 상태·버전 이력 목록·이전 버전 복구·가상 후보 미리보기(1B-4).
...
검증: `changeReason` 비어있지 않음, 배열에 **1개 이상** 항목(개수 자유 — 추가/삭제 가능), 각 항목의 `key`가 배열 안에서 서로 중복되지 않는 비어있지 않은 문자열, `label` 비어있지 않은 문자열, `points`가 0 이상의 숫자, **합계가 정확히 40**이 아니면 400 거부.

새 항목을 추가할 때 `key`는 화면에서 자동 생성한다(예: 타임스탬프 기반 임의 문자열) — 사람이 직접 영문 key를 입력할 필요는 없다. 기존 항목의 `key`는 이름을 나중에 바꿔도 그대로 유지된다(같은 항목이 라벨만 바뀐 것으로 취급 — 나중에 실제 평가 엔진이 생기면 이 key로 "어느 평가항목인지"를 추적하게 될 수 있어서, 라벨이 바뀌어도 항목 정체성은 유지하는 게 안전하다).
...
- **공통 40점 모달**: 항목별로 이름(텍스트)·배점(숫자) 입력칸이 있는 행이 나열되고, **"+ 항목 추가"** 버튼(빈 행 추가)과 각 행에 **"삭제"** 버튼이 있다 — 항목 수는 자유(최소 1개). 실시간 합계 표시("합계: 38/40" 처럼, 40이 아니면 저장 버튼 비활성화 또는 저장 시 서버가 400 반환)
...
- 항목을 자유롭게 추가/삭제/이름변경할 수 있게 되면서, "이 회사가 공통으로 중요하게 보는 역량이 무엇인지"에 대한 합의가 이전보다 느슨해질 수 있다 — 원본 명세서의 5개 항목(목표완결성·오너십 등)은 초기값일 뿐이고, 이후로는 화면에서 자유롭게 바뀔 수 있다는 걸 인지하고 있어야 한다(의도된 변경).
