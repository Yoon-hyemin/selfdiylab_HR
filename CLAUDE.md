...
- **이번 범위에 명시적으로 안 넣은 것들**: 후보 상세보기, 수동 평가·정보부족·중복 표시, 일시정지·재개, 체크포인트, 배치 시뮬레이션, 플랫폼 순서 제안, 소진 보고서, 대시보드 카드의 오늘/누적 추천 숫자 연동 — 전부 1E-2 이후 단계.
...
**비개발자 운영 메모**: 이제 승인된 검색 프로젝트를 열면 "검색 진행 보기" 버튼이 보인다. 누르면 가상의 후보들("가상후보-001" 같은 이름 — 진짜 사람 아님)을 만들어서, 지금 채점 기준으로 몇 명이 추천/확인필요/제외로 나오는지 미리 볼 수 있다. 아직 후보 한 명 한 명을 열어보거나 직접 평가하는 기능은 없다.
