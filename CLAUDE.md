...
- **DOM 선택자는 아직 미검증 placeholder다** — `chrome-extension/list-content-lib.js`/`list-content.js`의 카드 선택자(`[data-field="..."]`, `CANDIDATE_CARD_SELECTOR` 등)는 이 작업 시점에 실제 사람인 검색결과 리스트 페이지(`https://www.saramin.co.kr/zf_user/memcom/talent-pool/...`)를 라이브로 열어보지 못한 채(샌드박스 환경) 합리적 추정으로 작성됐다 — 실행엔진 OCR MVP 때 상세 페이지 선택자를 확정할 때와 같은 상황이다. **실사용 전 반드시 실제 화면을 열어 선택자를 확인·수정해야 한다** — 지금 상태로는 파싱이 0명으로 조용히 끝날 수 있다(다만 위 blocked 플래그 덕분에 최소한 "로그인 화면이라서"는 아니라는 것은 구분된다).
