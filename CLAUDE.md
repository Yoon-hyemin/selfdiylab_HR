...
- **프로덕션 마이그레이션 미적용 상태는 그대로**: `sql/015_talent_search.sql`(Phase 1A)과 `sql/016_talent_search_policy.sql`(Phase 1B-1)은 여전히 `development` 브랜치에만 적용돼 있다 — 이번 단계와는 무관하게 계속 미적용이다. `master` 배포 전 프로덕션 브랜치에도 실행 필요(사용자 확인 후).
