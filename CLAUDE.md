...
- `members.role`은 `관리자`/`부서장`/`팀원` 중 하나(기본값 `팀원`, DB CHECK 없이 앱 레벨 검증). 🎯목표 탭이 이 값에 따라 다르게 보인다: 회사 목표 생성은 관리자만, 부서 목표 생성은 그 팀 부서장만, 조회(전체 현황)는 로그인 없이 누구나. 이건 "막기 위한 보안"이 아니라 "각자한테 맞는 화면/버튼을 자동으로 보여주기" 위한 구분이라는 게 설계 의도다.
...
**비개발자 운영 메모**:
- 신규 구성원은 등록하면 자동으로 `role='팀원'`이다. 관리자·부서장 권한이 필요한 사람은 구성원 → 기본정보 수정 화면에서 인사팀이 직접 역할을 바꿔줘야 그 사람 화면에 해당 기능(예: 부서 목표 만들기 버튼)이 나타난다 — 자동으로 추론해주지 않는다.
