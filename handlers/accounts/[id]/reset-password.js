 ...
 *
 * POST { tempPassword? } -> 200 { tempPassword }
 *
 * 관리자가 직원의 비밀번호를 잊었을 때 초기화하는 절차(스펙 6번)를
 * 그대로 구현한다: 새 임시 비밀번호를 서버에서 생성(또는 관리자가 직접
 * 입력한 값 사용 -- handlers/accounts/index.js의 설명 참고) -> 기존 세션
 * 전부 종료(session_version 증가) -> must_change_password=true -> 실패
 * 횟수/잠금 초기화 -> 감사 로그 기록. 관리자는 기존 비밀번호를 조회할 수
 * 없다 -- 애초에 해시만 저장돼 있어서 "조회"라는 개념 자체가 없다.
 */
import { sql } from '../../_lib/db.js';
import { requireRole, generateTempPassword, hashPassword, validatePasswordPolicy } from '../../_lib/accountAuth.js';
...
export default async function handler(req, res) {
  ...
  const { id } = req.query;
  const { tempPassword: customTempPassword } = req.body || {};
